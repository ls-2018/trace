...
DEFINE_USDT_HOOK(struct ovs_operation_event *op; batch_process_op(OVS_OP_EXEC, event, &op);

		 if (op) {
		     u32 queue_id = op->queue_id;
		     u64 timestamp = ctx->timestamp;
		     if (bpf_map_update_elem(&flow_exec_tracking, &queue_id, &timestamp, BPF_NOEXIST)) {
