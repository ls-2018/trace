...
/* Hook placeholder */
__attribute__((noinline)) int hook0(struct user_ctx *ctx, struct retis_raw_event *event) {
    volatile int ret = 0;
    if (!ctx || !event)
        return 0;
    return ret;
...
static __always_inline int get_args(struct user_ctx *uctx, struct pt_regs *ctx) {
...
#define arg_case(x)                     \
    ...
    switch (cnt - 1) { arg_case(11) arg_case(10) arg_case(9) arg_case(8) arg_case(7) arg_case(6) arg_case(5) arg_case(4) arg_case(3) arg_case(2) arg_case(1) arg_case(0) }
    uctx->num = cnt;
